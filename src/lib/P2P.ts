...
export class P2P {
  ...
  private appendStream(userStream: MediaStream, callPeer: string, user: User) {
    ...
    div_2.className = "w-[200px] h-[150px] bg-slate-600 rounded-lg relative";
    ...
    div_3.className =
      "absolute inset-0 rounded-lg flex flex-col justify-end p-2 text-white";
  ...
  private socketEvents() {
    ...
    this.socket.on("userMuted", (user) => {
      const i = document.getElementById(`mic-${user.userId}`) as HTMLDivElement;
    ...
    this.socket.on("userCameraOff", (user) => {
      const div_1 = document.getElementById(
        `div-${user.userId}`
      ) as HTMLDivElement;

      const div_2 = document.createElement("div");
      div_2.id = `camera-off-${user.userId}`;
      div_2.className =
